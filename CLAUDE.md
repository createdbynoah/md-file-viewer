...
- Triage (`public/js/triage.js`) runs inside `PUT /api/files/:id` with old and new source in hand, wrapped in try/catch — a triage failure must never fail a save. No fuzzy matching: `fix`/`cut` found (whitespace/typographer-tolerant) → carried, gone → `addressed` with `replacedBy` found (`replacedSpan()`) between the surviving prefix/suffix context — a neighboring edit (same line or the next) can fall inside that window without touching the quote itself, so it retries with a shorter, closer-to-the-quote slice: full context, then the last/first 16 chars, then 8, before giving up and simply omitting `replacedBy` rather than guessing; `keep` must be byte-identical or it becomes `violated` (restored automatically if the text comes back); `q`/`general` are manual. Duplicated text follows one shared rule for every tag: if the quote occurred more than once in the old source, or does in the new one, a surviving hit counts as found only while it still carries matching context — the full stored prefix/suffix, else the 8 characters closest to the quote (both sides when both were captured) — otherwise the anchor is gone; a legacy anchor with no stored context at all falls back to the nearest hit by line. Sources holding any `\r` are compared `\r`-stripped on both sides (a textarea save rewrites CRLF to LF; dropping `\r` never changes a line number), an anchor that was already unlocatable in the OLD source stays open rather than being called addressed, and a block/approx anchor whose `rev` is not the previous revision is held at its clamped lines (its base is not what it was resolved against). A resolved item's `resolvedLines` are re-pinned on later revisions — following `replacedBy`, else re-running `replacedSpan` — with `linesRev` recording the revision they were pinned against. PUT skips triage entirely when `content.length * items.length > TRIAGE_BUDGET` (`5e8`, ≈ 1 MB × 500 comments), logging `comments.triageSkipped` and answering `triage: null` with the comments untouched. The client only renders the result: `repaint()` draws the banner before the rail so the rail can measure against the banner's final height, and the banner is dismissed per `localStorage` key `triageSeen:{id}:{rev}` (capped at 50 entries). After a save, `app.js` calls `comments.load()` before `exitEditMode()`, and `refresh()` no-ops while a load is in flight, so there is no "anchor not found" flash. The ▾ copy-options menu closes other toolbar menus and closes on Escape; the CriticMarkup export lists violated keeps at the top (`{>>k2 keep VIOLATED — restore exactly: "…" (near L4)<<}`), merges identical highlight ranges, degrades overlapping ones to a trailing `~"quote"` note, and neutralizes CriticMarkup delimiters in user text; an addressed item's exported line ends with `(addressed in rev N)`, plus `; L as of rev N` when its lines could not be re-pinned in the latest triage, and a VIOLATED line carries `(now "…")` with the text that stands there now.
